...
void test_compute_and_gate_check_huge_random_circuit_JQv2(OSTriple<BoolIO<NetIO>> *os, BoolIO<NetIO> *io, int len, int repeat) {
...
void test_ostriple(BoolIO<NetIO> *ios[threads + 1], int party, int len, int repeat) {
  //bool flag = true;
  auto t1 = clock_start();
  ...
  test_compute_and_gate_check_huge_random_circuit_JQv2(&os, ios[0], len, repeat);
}
...
int main(int argc, char **argv) {
  ...
  test_ostriple(ios, party, num, num1);
  for (int i = 0; i < threads; ++i) {
    delete ios[i]->io;
    delete ios[i];
  }
