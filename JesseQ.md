...
另一研究方向针对特定结构电路（如表示析取式$\mathcal{C}_1(w)=1 \mathcal{C}_2(w)=1 \mathcal{C}_B(w)=1$的$B$分支子电路）进行优化。最先进的Batchman[15]实现了批量析取式的亚线性通信和计算，其框架可通过黑盒方式实例化基于VOLE的常数轮ZKPs来处理乘法运算。
|      Boolean      |            |             |    Arithmetic     |            |             |
| :---------------: | :--------: | :---------: | :---------------: | :--------: | :---------: |
