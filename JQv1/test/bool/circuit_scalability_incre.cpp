...
void test_compute_and_gate_check_JQv1(OSTriple<BoolIO<NetIO>> *os,
                                 BoolIO<NetIO> *io, int input_sz_lg, int chunk_lg) {
  PRG prg;
  // long long len = 300000000;
  // int chunk = 30000000;
  // long long test_n = 1 << input_sz_lg;
  // long long chunk = 1 << chunk_lg;
  long long len = input_sz_lg;
  long long chunk = chunk_lg;
  bool cpu_flag = false;
...
void test_circuit_zk(BoolIO<NetIO> *ios[threads + 1], int party, int num, int num1) {
  auto t1 = clock_start();
  OSTriple<BoolIO<NetIO>> os(party, threads, ios);
  cout << party << "\tconstructor\t" << time_from(t1) << " us" << endl;


  test_compute_and_gate_check_JQv1(&os, ios[0], num, num1);
}
...
int main(int argc, char **argv) {
  ...
  test_circuit_zk(ios, party, num, num1);
  for (int i = 0; i < threads; ++i) {
    delete ios[i]->io;
    delete ios[i];
  }
